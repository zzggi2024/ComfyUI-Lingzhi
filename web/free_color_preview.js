...
const PREVIEW_EVENT = "lingzhi.free_color.preview";
...
function findFreeColorNodes() {
    ...
    return graphNodes.filter((node) => node?.type === "Lingzhi_FreeColor");
}
...
function renderLocalPreview(node) {
    if (!node || !node.__lingzhiFreeColorBaseImageData || !node.__lingzhiFreeColorPreviewCanvas) {
        return;
    }
    const base = node.__lingzhiFreeColorBaseImageData;
    ...
    const mask = node.__lingzhiFreeColorMaskImageData;
    ...
    const ctx = node.__lingzhiFreeColorPreviewCanvas.getContext("2d");
...
function scheduleLocalPreview(node) {
    ...
    if (!node.__lingzhiHasRealtimeBase) {
        return;
    }
    if (node.__lingzhiFreeColorPreviewTimer) {
        clearTimeout(node.__lingzhiFreeColorPreviewTimer);
    }
    node.__lingzhiFreeColorPreviewTimer = setTimeout(() => {
        node.__lingzhiFreeColorPreviewTimer = null;
        renderLocalPreview(node);
    }, 16);
}

function installRealtimeHooks(node) {
    if (!node || node.__lingzhiRealtimeHooksInstalled) {
        return;
    }
    node.__lingzhiRealtimeHooksInstalled = true;
    const widgets = node.widgets || [];
    for (const widget of widgets) {
        if (!widget || widget.type === "button" || widget.__lingzhiRealtimeWrapped) {
            continue;
        }
        ...
        widget.__lingzhiRealtimeWrapped = true;
    }
}
...
function syncWidgetSnapshot(node) {
    node.__lingzhiFreeColorWidgetSnapshot = buildWidgetSnapshot(node);
}
...
function handleSnapshotDrivenSync(node) {
    ...
    const previous = node.__lingzhiFreeColorWidgetSnapshot;
    const current = buildWidgetSnapshot(node);
    if (!previous) {
        node.__lingzhiFreeColorWidgetSnapshot = current;
        return;
    }
...
function startGlobalFreeColorSyncLoop() {
    if (window.__lingzhiFreeColorSyncLoopStarted) {
        return;
    }
    window.__lingzhiFreeColorSyncLoopStarted = true;
    window.setInterval(() => {
...
function applyPreviewToNode(node, detail) {
    ...
    if (!node.__lingzhiFreeColorPreviewCanvas) {
        node.__lingzhiFreeColorPreviewCanvas = document.createElement("canvas");
    }
    const outputImg = new Image();
    outputImg.onload = () => {
        node.__lingzhiFreeColorPreviewCanvas.width = outputImg.width;
        node.__lingzhiFreeColorPreviewCanvas.height = outputImg.height;
        const previewCtx = node.__lingzhiFreeColorPreviewCanvas.getContext("2d");
    ...
    baseImg.onload = () => {
        ...
        node.__lingzhiFreeColorBaseImageData = baseCtx.getImageData(0, 0, baseImg.width, baseImg.height);
        node.__lingzhiHasRealtimeBase = true;
        if (maskSrc) {
            ...
            maskImg.onload = () => {
                ...
                node.__lingzhiFreeColorMaskImageData = maskCtx.getImageData(0, 0, baseImg.width, baseImg.height);
            };
            maskImg.src = maskSrc;
        } else {
            node.__lingzhiFreeColorMaskImageData = null;
        }
...
function getPresetStore(node) {
    ...
    if (!node.properties.__lingzhiFreeColorPresets || typeof node.properties.__lingzhiFreeColorPresets !== "object") {
        node.properties.__lingzhiFreeColorPresets = { order: [], presets: {} };
    }
    const store = node.properties.__lingzhiFreeColorPresets;
...
app.registerExtension({
    name: "Lingzhi.FreeColorPreview",
    async setup() {
        if (!window.__lingzhiFreeColorPointerTrackInstalled) {
            window.__lingzhiFreeColorPointerTrackInstalled = true;
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name !== "Lingzhi_FreeColor") {
            return;
        }
        const onNodeCreated = nodeType.prototype.onNodeCreated;
        nodeType.prototype.onNodeCreated = function () {
            const r = onNodeCreated ? onNodeCreated.apply(this, arguments) : undefined;
            this.__lingzhiFreeColorBaseHeight = this.size ? this.size[1] : 200;
        ...
        nodeType.prototype.onDrawBackground = function (ctx) {
            ...
            const canvas = this.__lingzhiFreeColorPreviewCanvas;
