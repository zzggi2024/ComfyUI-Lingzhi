...
const NODE_TYPE = "Lingzhi_H3_Video";
...
function ensureDescriptionPanel(node) {
    if (node.__lingzhiH3Description || typeof node.addDOMWidget !== "function") {
        return node.__lingzhiH3Description || null;
    }
    ...
    const domWidget = node.addDOMWidget(
        "模型选择说明",
        "lingzhi_h3_model_description",
        root,
        { serialize: false, hideOnZoom: false },
    );
    domWidget.serialize = false;
    domWidget.computeSize = () => [0, 180];
    node.__lingzhiH3Description = { root, current, details, domWidget };
    return node.__lingzhiH3Description;
}
