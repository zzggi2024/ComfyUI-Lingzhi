...
function updateWarningStatus(node) {
  ...
  const baseTitle = node.__lingzhiEnhancerBaseTitle || node.title || DISPLAY_NAME;
  node.__lingzhiEnhancerBaseTitle = baseTitle.replace(/ · 已连接素材不参与$/, "");
  node.title = hasWarning
    ? `${node.__lingzhiEnhancerBaseTitle} · 已连接素材不参与`
    : node.__lingzhiEnhancerBaseTitle;
}
...
function setModelRefreshStatus(node, message) {
  ...
  node.__lingzhiEnhancerModelStatusVisible = Boolean(widget.value);
}

function applyNodePolicy(node, { conservative = false } = {}) {
  if (!node || node.__lingzhiEnhancerApplying) return;
  node.__lingzhiEnhancerApplying = true;
  try {
    ...
    for (const widget of node.widgets || []) {
      setWidgetVisibility(
        ...
        widget.name === ADVANCED_WIDGET
          ...
          || (widget.name === MODEL_LIST_WIDGET && node.__lingzhiEnhancerModelListLoaded)
          || (widget.name === MODEL_STATUS_WIDGET && node.__lingzhiEnhancerModelStatusVisible)
    ...
    node.__lingzhiEnhancerApplying = false;
  }
}
...
function updateModelListOptions(node, models) {
  ...
  node.__lingzhiEnhancerModelListLoaded = true;
}

function setModelLocked(node, locked) {
  node.__lingzhiModelLocked = Boolean(locked);
...
function modelRefreshQueue(node) {
  if (node.__lingzhiEnhancerModelQueue) return node.__lingzhiEnhancerModelQueue;
  node.__lingzhiEnhancerModelQueue = createLatestRequestQueue(async (request, isLatest) => {
    try {
      ...
      console.warn("[Lingzhi] 多模型提示词增强器暂时无法刷新模型列表");
      return null;
    } finally {
      if (isLatest()) node.setDirtyCanvas?.(true, true);
    }
  });
  return node.__lingzhiEnhancerModelQueue;
}
...
function installWidgetCallbacks(node) {
  for (const widget of node.widgets || []) {
    if (!widget || widget.__lingzhiEnhancerCallbackInstalled) continue;
    widget.__lingzhiEnhancerCallbackInstalled = true;
    const original = widget.callback;
    widget.callback = function (value) {
      const result = original ? original.apply(this, arguments) : undefined;
      if (widget.name === MODEL_WIDGET && !node.__lingzhiEnhancerSettingModel) {
