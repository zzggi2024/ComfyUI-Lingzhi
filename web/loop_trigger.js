...
const NODE_CLASS_NAMES = new Set(["Lingzhi_Loop_Trigger", "LZ-Lingzhi_Loop_Trigger"]);
...
function setWidgetHiddenState(widget, hidden) {
	...
	} else if (widget.__lingzhiOriginalComputeSize) {
		widget.type = widget.__lingzhiOriginalType;
		widget.computeSize = widget.__lingzhiOriginalComputeSize;
	}
}
...
async function scanFolder(path, includeSubdirs = true) {
	const response = await api.fetchApi("/lingzhi/loop_trigger/scan", {
		method: "POST",
...
async function stopLoopTasks(node) {
	...
	try {
		...
		console.warn("[Lingzhi] 停止未执行任务失败:", error);
	}
...
async function initializeFolder(node, path, includeSubdirs = true) {
	const response = await api.fetchApi("/lingzhi/loop_trigger/initialize", {
		method: "POST",
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({
			path,
...
async function syncNodeFolderState(node, options = {}) {
	...
	if (!silent && fromInit) {
		console.log("[Lingzhi] 循环触发初始化诊断:", result);
...
async function pollLoopTriggerNodes() {
	...
	for (const [nodeId] of Array.from(watchedNodes.entries())) {
		...
		try {
			...
			console.warn("[Lingzhi] 循环触发自动初始化失败:", error);
		}
	}
}
...
function startPolling() {
	...
	pollTimer = window.setInterval(() => {
		pollLoopTriggerNodes().catch((error) => {
			console.warn("[Lingzhi] 循环触发轮询失败:", error);
		});
	}, 2000);
}
	...
	try {
		for (let attempt = 1; attempt <= 3; attempt++) {
			...
			try {
				...
				console.warn(`[Lingzhi] 循环触发续队列失败，第 ${attempt} 次:`, error);
				if (attempt < 3) {
					await sleep(1000);
				}
			}
...
function bindLoopTriggerEvents() {
	if (!feedbackListenerBound) {
		api.addEventListener("lingzhi.loop_trigger.feedback", (evt) => {
	...
	if (!queueListenerBound) {
		api.addEventListener("lingzhi.loop_trigger.add_queue", async (evt) => {
			const detail = evt?.detail ?? null;
			const nodeId = detail?.node_id ? String(detail.node_id) : "";
...
function enhanceLoopTriggerNode(node) {
	if (node.__lingzhiLoopTriggerEnhanced) {
		return;
	}
	node.__lingzhiLoopTriggerEnhanced = true;
		...
		pathWidget.callback = async (...args) => {
			...
			if (Boolean(state?.initialized)) {
				try {
					...
					console.warn("[Lingzhi] 循环触发路径更新失败:", error);
				}
			}
		};
	}
...
app.registerExtension({
	name: "Lingzhi.LoopTrigger",
	async setup() {
		bindLoopTriggerEvents();
		startPolling();
	},
	async beforeRegisterNodeDef(nodeType, nodeData) {
		if (!NODE_CLASS_NAMES.has(nodeData?.name)) {
			return;
		}
