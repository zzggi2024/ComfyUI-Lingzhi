...
const FONT_STYLE_NODE_NAME = "Lingzhi_FontStyleSelector";
const STYLE_CACHE = {
    list: null,
    promise: null,
};
...
function ensureSelectorStyles() {
    if (document.getElementById("lingzhi-font-style-selector-style")) {
        return;
    }

    const style = document.createElement("style");
    style.id = "lingzhi-font-style-selector-style";
    style.textContent = `
        .lingzhi-font-style-root {
            display: flex;
        ...
        .lingzhi-font-style-replace-input {
            width: 100%;
            min-height: 44px;
            max-height: 180px;
            padding: 10px 12px;
        ...
        .lingzhi-font-style-toolbar {
            display: flex;
            gap: 8px;
            align-items: center;
        }
        .lingzhi-font-style-search {
            flex: 1;
            min-width: 0;
            padding: 6px 8px;
            border: 1px solid rgba(255, 255, 255, 0.15);
        ...
        .lingzhi-font-style-button:hover {
            background: rgba(255, 255, 255, 0.14);
        }
        .lingzhi-font-style-status {
            opacity: 0.8;
            white-space: nowrap;
        }
        .lingzhi-font-style-body {
            display: flex;
            gap: 10px;
            min-height: 0;
            flex: 1;
        }
        .lingzhi-font-style-list {
            flex: 1.15;
            min-width: 0;
            overflow-y: auto;
            display: flex;
        ...
        .lingzhi-font-style-card:hover {
            background: rgba(255, 255, 255, 0.12);
            transform: translateY(-1px);
        }
        .lingzhi-font-style-card.is-selected {
            border-color: rgba(93, 199, 112, 0.9);
            background: rgba(93, 199, 112, 0.18);
        }
        .lingzhi-font-style-check {
            width: 14px;
            height: 14px;
            border-radius: 50%;
        ...
        .lingzhi-font-style-card.is-selected .lingzhi-font-style-check {
            background: #5dc770;
            border-color: #5dc770;
        }
        .lingzhi-font-style-name {
            line-height: 1.2;
        }
        .lingzhi-font-style-preview {
            flex: 0.95;
            min-width: 250px;
            display: flex;
            align-items: center;
        ...
        .lingzhi-font-style-preview-image {
            width: 100%;
            aspect-ratio: 1.45 / 1;
            object-fit: contain;
            border-radius: 8px;
...
async function getFontStyles() {
    ...
    STYLE_CACHE.promise = (async () => {
        const response = await api.fetchApi("/lingzhi/font_styles/list");
...
function createSelectorUI(node, replaceWidget, selectionWidget) {
    ...
    const root = createElement("div", "lingzhi-font-style-root");
    const replaceInput = createElement("textarea", "lingzhi-font-style-replace-input");
    replaceInput.rows = 1;
    replaceInput.placeholder = "替换文本输入";

    const toolbar = createElement("div", "lingzhi-font-style-toolbar");
    const searchInput = createElement("input", "lingzhi-font-style-search");
    searchInput.type = "text";
    searchInput.placeholder = "搜索风格...";

    const clearButton = createElement("button", "lingzhi-font-style-button", "清空");
    clearButton.type = "button";
    const status = createElement("div", "lingzhi-font-style-status", "加载中...");

    toolbar.append(searchInput, clearButton, status);

    const body = createElement("div", "lingzhi-font-style-body");
    const list = createElement("div", "lingzhi-font-style-list");
    const preview = createElement("div", "lingzhi-font-style-preview");
    const previewImage = createElement("img", "lingzhi-font-style-preview-image");
    ...
    function renderList() {
        ...
        if (!visibleStyles.length) {
            list.append(createElement("div", "lingzhi-font-style-empty", "没有匹配的风格"));
        ...
        for (const styleItem of visibleStyles) {
            const card = createElement("div", "lingzhi-font-style-card");
            if (state.selected.has(styleItem.name)) {
                card.classList.add("is-selected");
            }

            const check = createElement("div", "lingzhi-font-style-check");
            const name = createElement("div", "lingzhi-font-style-name", styleItem.name_cn || styleItem.name);
    ...
    getFontStyles()
        ...
        .catch(error => {
            ...
            list.innerHTML = "";
            list.append(createElement("div", "lingzhi-font-style-empty", `风格列表加载失败: ${error.message}`));
            updatePreview(null);
        });
...
app.registerExtension({
    name: "lingzhi.font_style_selector",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            if (!this.__lingzhiFontStyleWidget) {
                const root = createSelectorUI(this, replaceWidget, selectionWidget);
                this.__lingzhiFontStyleWidget = this.addDOMWidget("风格选择器", "lingzhi_font_style_selector", root);
            }
